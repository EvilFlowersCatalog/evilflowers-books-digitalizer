"""WebDAV backend for the scan shares.

Thin wrapper around `webdav4 <https://github.com/skshetry/webdav4>`_ exposing
the domain vocabulary (books, pages) instead of raw paths. Needs the VPN to
reach ``datasilo2.schk.sk``; in production the scans are a local mount and
:class:`~.filesystem.FilesystemBookSource` is used instead.
...
class BookSource(AbstractBookSource):
    ...
    def list_books(self) -> list[str]:
        ...
        names = (
            PurePosixPath(entry["name"]).name for entry in entries if entry["type"] == "directory"
        )
    ...
    def stage_book(
        ...
        for page in pages:
            ...
            if local_path.exists() and local_path.stat().st_size == page.size:
