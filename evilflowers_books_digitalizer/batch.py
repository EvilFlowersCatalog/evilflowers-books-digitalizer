...
after itself: the downloaded TIFFs, the preprocessed pages and the raw
(pre-OCR) PDF are deleted; only the final PDF + text sidecar remain.

Designed to run under ``concurrent.futures.ProcessPoolExecutor`` — OCRmyPDF's
API is not thread-safe, and separate processes also parallelize the OpenCV
preprocessing. Everything is constructed *inside* the worker from plain
string arguments, so nothing non-picklable crosses the process boundary.
"""
...
def process_book(
    ...
    ``jobs`` caps OCRmyPDF's internal parallelism — with N books in flight,
    pass roughly ``cpu_count // N``. ``min_free_gb`` guards the disk: the
    worker waits (up to ``disk_wait_minutes``) for other workers' cleanups
    to free space before starting a download.
    """
    ...
    try:
        ...
        pipeline = build_pipeline(source, cache, jobs=jobs)
        ...
        # low-storage mode: drop everything except the final PDF + sidecar
        if not keep_cache:
